(function() {(function WebXRPolyfillInjection() {
(function (global, factory) {
  ...
  const PRIVATE$1 = Symbol('@@webxr-polyfill/XRPose');
  class XRPose$1 {
    ...
    get emulatedPosition() { return this[PRIVATE$1].emulatedPosition; }
  }
  ...
  class XRView {
    ...
    get projectionMatrix() {
      return this[PRIVATE$5].device.getProjectionMatrix(this.eye, this[PRIVATE$5].viewIndex);
    }
  ...
  class XRFrame {
    ...
    getViewerPose(referenceSpace) {
      if (!this[PRIVATE$6].animationFrame) {
        throw new DOMException(NON_ANIMFRAME_MSG, 'InvalidStateError');
      }
      if (!this[PRIVATE$6].active) {
        throw new DOMException(NON_ACTIVE_MSG, 'InvalidStateError');
      }
      const device = this[PRIVATE$6].device;
      const session = this[PRIVATE$6].session;
      session[PRIVATE$d].viewerSpace._ensurePoseUpdated(device, this[PRIVATE$6].id);
      referenceSpace._ensurePoseUpdated(device, this[PRIVATE$6].id);
      let viewerTransform = referenceSpace._getSpaceRelativeTransform(session[PRIVATE$d].viewerSpace);
      const views = [];
      for (const viewSpace of session[PRIVATE$d].viewSpaces) {
        viewSpace._ensurePoseUpdated(device, this[PRIVATE$6].id);
        let viewTransform = referenceSpace._getSpaceRelativeTransform(viewSpace);
        let view = new XRView(device, viewTransform, viewSpace.eye, this[PRIVATE$6].sessionId, viewSpace.viewIndex);
        views.push(view);
      }
    ...
    getPose(space, baseSpace) {
      ...
      if (space._specialType === "target-ray" || space._specialType === "grip") {
        ...
        space._ensurePoseUpdated(device, this[PRIVATE$6].id);
        baseSpace._ensurePoseUpdated(device, this[PRIVATE$6].id);
  ...
  const PRIVATE$7 = Symbol('@@webxr-polyfill/XRSpace');
  class XRSpace {
    ...
    get _specialType() {
      return this[PRIVATE$7].specialType;
    }
    get _inputSource() {
      return this[PRIVATE$7].inputSource;
    }
    _ensurePoseUpdated(device, frameId) {
      if (frameId == this[PRIVATE$7].lastFrameId) return;
      this[PRIVATE$7].lastFrameId = frameId;
      this._onPoseUpdate(device);
    }
    _onPoseUpdate(device) {
      if (this[PRIVATE$7].specialType == 'viewer') {
        this._baseMatrix = device.getBasePoseMatrix();
      }
    }
    set _baseMatrix(matrix) {
      this[PRIVATE$7].baseMatrix = matrix;
      this[PRIVATE$7].inverseBaseMatrix = null;
    }
    get _baseMatrix() {
      if (!this[PRIVATE$7].baseMatrix) {
        if (this[PRIVATE$7].inverseBaseMatrix) {
          this[PRIVATE$7].baseMatrix = new Float32Array(16);
          invert(this[PRIVATE$7].baseMatrix, this[PRIVATE$7].inverseBaseMatrix);
        }
      }
      return this[PRIVATE$7].baseMatrix;
    }
    set _inverseBaseMatrix(matrix) {
      this[PRIVATE$7].inverseBaseMatrix = matrix;
      this[PRIVATE$7].baseMatrix = null;
    }
    get _inverseBaseMatrix() {
      if (!this[PRIVATE$7].inverseBaseMatrix) {
        if (this[PRIVATE$7].baseMatrix) {
          this[PRIVATE$7].inverseBaseMatrix = new Float32Array(16);
          invert(this[PRIVATE$7].inverseBaseMatrix, this[PRIVATE$7].baseMatrix);
        }
      }
      return this[PRIVATE$7].inverseBaseMatrix;
    }
  ...
  const PRIVATE$8 = Symbol('@@webxr-polyfill/XRReferenceSpace');
  ...
  class XRReferenceSpace extends XRSpace {
    ...
    _originOffsetMatrix() {
      return this[PRIVATE$8].originOffset;
    }
    _adjustForOriginOffset(transformMatrix) {
      let inverseOriginOffsetMatrix = new Float32Array(16);
      invert(inverseOriginOffsetMatrix, this[PRIVATE$8].originOffset);
    ...
    getOffsetReferenceSpace(additionalOffset) {
      let newSpace = new XRReferenceSpace(
        this[PRIVATE$8].type,
        this[PRIVATE$8].transform,
        this[PRIVATE$8].bounds);
      multiply(newSpace[PRIVATE$8].originOffset, this[PRIVATE$8].originOffset, additionalOffset.matrix);
      return newSpace;
    }
  }
  ...
  class XRRenderState {
    ...
    get depthNear() { return this[PRIVATE$9].config.depthNear; }
    get depthFar() { return this[PRIVATE$9].config.depthFar; }
    get inlineVerticalFieldOfView() { return this[PRIVATE$9].config.inlineVerticalFieldOfView; }
    get baseLayer() { return this[PRIVATE$9].config.baseLayer; }
  }

  const PRIVATE$a = Symbol('@@webxr-polyfill/XRInputSourceEvent');
  class XRInputSourceEvent extends Event {
    ...
    get frame() { return this[PRIVATE$a].frame; }
    get inputSource() { return this[PRIVATE$a].inputSource; }
  }

  const PRIVATE$b = Symbol('@@webxr-polyfill/XRSessionEvent');
  class XRSessionEvent extends Event {
    ...
    get session() { return this[PRIVATE$b].session; }
  }

  const PRIVATE$c = Symbol('@@webxr-polyfill/XRInputSourcesChangeEvent');
  class XRInputSourcesChangeEvent extends Event {
    ...
    get added() { return this[PRIVATE$c].added; }
    get removed() { return this[PRIVATE$c].removed; }
  }

  const PRIVATE$d = Symbol('@@webxr-polyfill/XRSession');
  ...
  class XRSession$1 extends EventTarget {
    constructor(device, mode, id) {
      ...
      this[PRIVATE$d].onDeviceFrame = () => {
        if (this[PRIVATE$d].ended || this[PRIVATE$d].suspended) {
          return;
        }
        this[PRIVATE$d].deviceFrameHandle = null;
        this[PRIVATE$d].startDeviceFrameLoop();
        if (this[PRIVATE$d].pendingRenderState !== null) {
          this[PRIVATE$d].activeRenderState = new XRRenderState(this[PRIVATE$d].pendingRenderState);
          this[PRIVATE$d].pendingRenderState = null;
          if (this[PRIVATE$d].activeRenderState.baseLayer) {
            this[PRIVATE$d].device.onBaseLayerSet(
              this[PRIVATE$d].id,
              this[PRIVATE$d].activeRenderState.baseLayer);
          }
        }
        if (this[PRIVATE$d].activeRenderState.baseLayer === null) {
          return;
        }
        const frame = new XRFrame(device, this, this[PRIVATE$d].id);
        const callbacks = this[PRIVATE$d].currentFrameCallbacks = this[PRIVATE$d].frameCallbacks;
        this[PRIVATE$d].frameCallbacks = [];
        frame[PRIVATE$6].active = true;
        frame[PRIVATE$6].animationFrame = true;
        this[PRIVATE$d].device.onFrameStart(this[PRIVATE$d].id, this[PRIVATE$d].activeRenderState);
        ...
        this[PRIVATE$d].currentFrameCallbacks = null;
        frame[PRIVATE$6].active = false;
        this[PRIVATE$d].device.onFrameEnd(this[PRIVATE$d].id);
      };
      this[PRIVATE$d].startDeviceFrameLoop = () => {
        if (this[PRIVATE$d].deviceFrameHandle === null) {
          this[PRIVATE$d].deviceFrameHandle = this[PRIVATE$d].device.requestAnimationFrame(
            this[PRIVATE$d].onDeviceFrame
          );
        }
      };
      this[PRIVATE$d].stopDeviceFrameLoop = () => {
        const handle = this[PRIVATE$d].deviceFrameHandle;
        if (handle !== null) {
          this[PRIVATE$d].device.cancelAnimationFrame(handle);
          this[PRIVATE$d].deviceFrameHandle = null;
        }
      };
      this[PRIVATE$d].onPresentationEnd = sessionId => {
        if (sessionId !== this[PRIVATE$d].id) {
          this[PRIVATE$d].suspended = false;
          this[PRIVATE$d].startDeviceFrameLoop();
          this.dispatchEvent('focus', { session: this });
          return;
        }
        this[PRIVATE$d].ended = true;
        this[PRIVATE$d].stopDeviceFrameLoop();
        device.removeEventListener('@@webxr-polyfill/vr-present-end', this[PRIVATE$d].onPresentationEnd);
        device.removeEventListener('@@webxr-polyfill/vr-present-start', this[PRIVATE$d].onPresentationStart);
        device.removeEventListener('@@webxr-polyfill/input-select-start', this[PRIVATE$d].onSelectStart);
        device.removeEventListener('@@webxr-polyfill/input-select-end', this[PRIVATE$d].onSelectEnd);
        this.dispatchEvent('end', new XRSessionEvent('end', { session: this }));
      };
      device.addEventListener('@@webxr-polyfill/vr-present-end', this[PRIVATE$d].onPresentationEnd);
      this[PRIVATE$d].onPresentationStart = sessionId => {
        if (sessionId === this[PRIVATE$d].id) {
          return;
        }
        this[PRIVATE$d].suspended = true;
        this[PRIVATE$d].stopDeviceFrameLoop();
        this.dispatchEvent('blur', { session: this });
      };
      device.addEventListener('@@webxr-polyfill/vr-present-start', this[PRIVATE$d].onPresentationStart);
      this[PRIVATE$d].onSelectStart = evt => {
        if (evt.sessionId !== this[PRIVATE$d].id) {
          return;
        }
        this[PRIVATE$d].dispatchInputSourceEvent('selectstart',  evt.inputSource);
      };
      device.addEventListener('@@webxr-polyfill/input-select-start', this[PRIVATE$d].onSelectStart);
      this[PRIVATE$d].onSelectEnd = evt => {
        if (evt.sessionId !== this[PRIVATE$d].id) {
          return;
        }
        this[PRIVATE$d].dispatchInputSourceEvent('select',  evt.inputSource);
        this[PRIVATE$d].dispatchInputSourceEvent('selectend',  evt.inputSource);
      };
      device.addEventListener('@@webxr-polyfill/input-select-end', this[PRIVATE$d].onSelectEnd);
      this[PRIVATE$d].onSqueezeStart = evt => {
        if (evt.sessionId !== this[PRIVATE$d].id) {
          return;
        }
        this[PRIVATE$d].dispatchInputSourceEvent('squeezestart',  evt.inputSource);
      };
      device.addEventListener('@@webxr-polyfill/input-squeeze-start', this[PRIVATE$d].onSqueezeStart);
      this[PRIVATE$d].onSqueezeEnd = evt => {
        if (evt.sessionId !== this[PRIVATE$d].id) {
          return;
        }
        this[PRIVATE$d].dispatchInputSourceEvent('squeezeend',  evt.inputSource);
        this[PRIVATE$d].dispatchInputSourceEvent('squeeze',  evt.inputSource);
      };
      device.addEventListener('@@webxr-polyfill/input-squeeze-end', this[PRIVATE$d].onSqueezeEnd);
      this[PRIVATE$d].dispatchInputSourceEvent = (type, inputSource) => {
        const frame = new XRFrame(device, this, this[PRIVATE$d].id);
    ...
    get renderState() { return this[PRIVATE$d].activeRenderState; }
    get environmentBlendMode() {
      return this[PRIVATE$d].device.environmentBlendMode || 'opaque';
    }
    async requestReferenceSpace(type) {
      if (this[PRIVATE$d].ended) {
        return;
      }
      if (!XRReferenceSpaceTypes.includes(type)) {
        throw new TypeError(`XRReferenceSpaceType must be one of ${XRReferenceSpaceTypes}`);
      }
      if (!this[PRIVATE$d].device.doesSessionSupportReferenceSpace(this[PRIVATE$d].id, type)) {
        throw new DOMException(`The ${type} reference space is not supported by this session.`, 'NotSupportedError');
      }
      if (type === 'viewer') {
        return this[PRIVATE$d].viewerSpace;
      }
      let transform = await this[PRIVATE$d].device.requestFrameOfReferenceTransform(type);
      if (type === 'bounded-floor') {
        if (!transform) {
          throw new DOMException(`${type} XRReferenceSpace not supported by this device.`, 'NotSupportedError');
        }
        let bounds = this[PRIVATE$d].device.requestStageBounds();
        if (!bounds) {
          throw new DOMException(`${type} XRReferenceSpace not supported by this device.`, 'NotSupportedError');
        }
    ...
    requestAnimationFrame(callback) {
      ...
      const handle = ++this[PRIVATE$d].frameHandle;
      this[PRIVATE$d].frameCallbacks.push({
        handle,
        callback,
        cancelled: false
      });
    ...
    cancelAnimationFrame(handle) {
      let callbacks = this[PRIVATE$d].frameCallbacks;
      let index = callbacks.findIndex(d => d && d.handle === handle);
      if (index > -1) {
        callbacks[index].cancelled = true;
        callbacks.splice(index, 1);
      }
      callbacks = this[PRIVATE$d].currentFrameCallbacks;
      if (callbacks) {
    ...
    get inputSources() {
      return this[PRIVATE$d].device.getInputSources();
    }
    async end() {
      if (this[PRIVATE$d].ended) {
        return;
      }
      if (this[PRIVATE$d].immersive) {
        this[PRIVATE$d].ended = true;
        this[PRIVATE$d].device.removeEventListener('@@webxr-polyfill/vr-present-start',
                                                   this[PRIVATE$d].onPresentationStart);
        this[PRIVATE$d].device.removeEventListener('@@webxr-polyfill/vr-present-end',
                                                   this[PRIVATE$d].onPresentationEnd);
        this[PRIVATE$d].device.removeEventListener('@@webxr-polyfill/input-select-start',
                                                   this[PRIVATE$d].onSelectStart);
        this[PRIVATE$d].device.removeEventListener('@@webxr-polyfill/input-select-end',
                                                   this[PRIVATE$d].onSelectEnd);
        this.dispatchEvent('end', new XRSessionEvent('end', { session: this }));
      }
      this[PRIVATE$d].stopDeviceFrameLoop();
      return this[PRIVATE$d].device.endSession(this[PRIVATE$d].id);
    }
    updateRenderState(newState) {
      ...
      if (this[PRIVATE$d].pendingRenderState === null) {
        const activeRenderState = this[PRIVATE$d].activeRenderState;
        this[PRIVATE$d].pendingRenderState = {
      ...
      Object.assign(this[PRIVATE$d].pendingRenderState, newState);
    }
    _checkInputSourcesChange() {
      const added = [];
      const removed = [];
      const newInputSources = this.inputSources;
      const oldInputSources = this[PRIVATE$d].currentInputSources;
      ...
      this[PRIVATE$d].currentInputSources.length = 0;
      for (const newInputSource of newInputSources) {
        this[PRIVATE$d].currentInputSources.push(newInputSource);
      }
    }
  }
...
to native implementations of the API.`;
  ...
  class XRWebGLLayer {
    ...
    static getNativeFramebufferScaleFactor(session) {
      ...
      if (session[PRIVATE$d].ended) { return 0.0; }
      return 1.0;
    }
  }
  ...
  const PRIVATE$i = Symbol('@@webxr-polyfill/XRHitTestResult');
  class XRHitTestResult {
    ...
    getPose(baseSpace) {
      ...
      space._baseMatrix = copy$4(create$6(), this[PRIVATE$i].transform.matrix);
      return this[PRIVATE$i].frame.getPose(space, baseSpace);
    }
  }
  ...
  class XRTransientInputHitTestResult {
    ...
    get inputSource() {
      return this[PRIVATE$l].inputSource;
    }
    get results() {
      return this[PRIVATE$l].results;
    }
  }
  ...
  const PRIVATE$n = Symbol('@@webxr-polyfill/XRRemappedGamepad');
  ...
  class XRRemappedGamepad {
    ...
    _update() {
      let gamepad = this[PRIVATE$n].gamepad;
      let map = this[PRIVATE$n].map;
      let axes = this[PRIVATE$n].axes;
      ...
      let buttons = this[PRIVATE$n].buttons;
    ...
    get _profiles() {
      return this[PRIVATE$n].profiles;
    }
    get index() {
      return -1;
    }
    get connected() {
      return this[PRIVATE$n].gamepad.connected;
    }
    get timestamp() {
      return this[PRIVATE$n].gamepad.timestamp;
    }
    get mapping() {
      return this[PRIVATE$n].mapping;
    }
    get axes() {
      return this[PRIVATE$n].axes;
    }
    get buttons() {
      return this[PRIVATE$n].buttons;
    }
    get hapticActuators() {
      return this[PRIVATE$n].gamepad.hapticActuators;
    }
  }
    ...
    getXRPose(coordinateSystem, poseType) {
      ...
      switch(poseType) {
        case "target-ray":
          ...
          if (this.gamepad && this.gamepad[PRIVATE$n].targetRayTransform) {
            multiply(this.outputMatrix, this.outputMatrix, this.gamepad[PRIVATE$n].targetRayTransform);
          }
          break;
        case "grip":
          if (!this.nativeGamepad || !this.nativeGamepad.pose) {
          ...
          coordinateSystem._transformBasePoseMatrix(this.outputMatrix, this.basePoseMatrix);
          if (this.gamepad && this.gamepad[PRIVATE$n].gripTransform) {
            multiply(this.outputMatrix, this.outputMatrix, this.gamepad[PRIVATE$n].gripTransform);
          }
          break;
        default:
          return null;
  ...
  class EmulatedXRDevice extends XRDevice {
  	...
  	onFrameStart(sessionId, renderState) {
  		...
  		invert$2(
  			this.leftViewMatrix,
  			translateOnX(copy$4(this.leftViewMatrix, this.matrix), -0.02),
  		);
  		invert$2(
  	...
  	_isPointerCloseEnoughToTablet() {
  		...
  		const matrix = fromRotationTranslation$1(
  			create$6(),
  			pose.orientation,
  			pose.position,
  		);
...
`);
  ...
  class CustomWebXRPolyfill extends WebXRPolyfill {
  	constructor() {
  				...
  				.then((session) => {
  					if (mode === 'immersive-vr' || mode === 'immersive-ar') {
  						...
  						if (
  							...
  							const device = session[PRIVATE$d].device;
  		...
  		XRSession$1.prototype.requestHitTestSourceForTransientInput = function (
  			options,
  		) {
  		...
  		XRFrame.prototype.getHitTestResults = function (hitTestSource) {
  			const device = this.session[PRIVATE$d].device;
  ...
  const requestXRDevice$1 = async (global, _config) => {
